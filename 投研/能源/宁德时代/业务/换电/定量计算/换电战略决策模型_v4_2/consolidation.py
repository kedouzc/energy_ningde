...
def build_consolidated_ledger(
    ...
    # 换电主值=运营主体价值+护价+锁定订单量贡献；充电段份额净效应不进主值。
    # 换电段锁定订单按外部权益口径：40%内部权益电池的价值已在运营线确认，全额计入会双算。
    total_np_increment = swap_margin_np + swap_locked_np + direct_np
    total_value_increment = (swap_margin_np + swap_locked_np) * pe + direct_value
