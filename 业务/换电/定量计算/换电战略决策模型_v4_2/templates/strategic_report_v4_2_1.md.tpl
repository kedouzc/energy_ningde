...
> 本报告由 `$model_name` 自动生成。外部参数集中在 `configs/base.toml`，正文数字来自 `outputs/decision_snapshot_v4_2.json`；模板不保存业务结果数字。报告按“问题—方法—参数—推导—判断—质疑—结论”组织，可以脱离 Python 独立阅读。
...
H1财务分摊法回答“当前$current_market_cap_text A+H市值中，动力电池分部约贡献多少”，是本报告的主基准；${current_shipments_gwh}GWh量价法回答“与${target_year}电动车制造盘同口径比较，${reference_year}制造价值是多少”，降为道路口径锚。两种方法的差额只是估算口径差，没有公开证据支持把它直接归入船舶、eVTOL或机器人，故不编造归因。
...
本报告采用**节点四舍五入**：年度队列、折现和价格曲线保留计算精度；进入终局站网、CAPEX和经营模型前，各车型累计车辆数保留小数点后$terminal_vehicle_decimals位、加权频次保留小数点后$terminal_frequency_decimals位。正文金额显示到小数点后$report_money_decimals位。若每乘一步都四舍五入，多年累计会系统性放大或缩小。
...
#### 2.2.1 电池寿命由使用强度反推：为什么大车更短、小车更长

电池不是"按车型规定能用到哪一年"，而是**被用坏的**：累计循环次数达到临界点就退出换电网络。因此寿命必须由各车型的实际使用强度反推，而不是按车型拍一个经验年数。

推算口径：以磷酸铁锂SOH衰减的物理临界点（约$critical_cycles次循环、SOH≈82–85%，用户体感明显感知续航缩短的时点）为触发条件，再按该车型的换电频次反推需要几年走到这一步；低频场景叠加日历寿命封顶（$calendar_cap_years年）——即便循环没到，静置衰减与技术迭代也会让电池退出。
...
每个电池批次按自己的初装年份、寿命和届时价格计算更新；更新净支出按WACC折回初装时点，形成全周期资本倍数（EAC＝1＋净重置比例，方法沿用v3.2 §1.2.1折扣链）。第$model_horizon_years年末仍在役电池的可回收价值拆成：
...
$performance_anchor_table

再看与${reference_year}基准的量级比较：
...
$swap_multiple是含战略溢价的基础设施运营倍数：同一笔换电现金流只在“重资产运营线”估一次价值，制造利润与运营权益分开算，不存在第三层资管平台价值。
...
### A.2 电池寿命口径变更的影响：v3.2拍值 vs 按使用强度推算

本节把同一套模型、同一套输入，分别用两套寿命口径各完整重跑一次，以量化"寿命从按车型拍值改为按使用强度推算"到底改变了多少。左列为v3.2遗留硬编码寿命，右列为本报告基准口径。

$life_mode_comparison_table

$life_mode_comparison_note
