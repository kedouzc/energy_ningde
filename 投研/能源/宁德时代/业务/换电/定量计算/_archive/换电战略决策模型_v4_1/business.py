...
def build_swap_business(config: dict, scale: ScaleResult, capex: CapexResult) -> SwapBusinessResult:
    ...
    # 租金口径按v3修订链拆开：车端运营电池全量计租；站内周转电池只把
    # 外部经济权益对应部分视为对外电池资产服务，避免40%内部权益重复确认。
    station_external_rent_gwh = (
        station_battery_gwh * business["swap_battery_external_sales_share"]
    )
    rent_gwh = cumulative_vehicle_gwh + station_external_rent_gwh
    ...
    labor = (
        scale.mature_daily_swaps["heavy"]
        * business["operating_days"]
        * business["heavy_labor_rmb_swap"]
        + scale.mature_daily_swaps["choco"]
        * business["operating_days"]
        * business["passenger_labor_rmb_swap"]
    ) / 1e8
    ...
    forward_fcff = ebitda * (1 - finance["tax_rate"]) + depreciation * finance["tax_rate"]
    minimum_distributable = max(0.0, capex.annual_capital_requirement_yi - interest)
    catl_minimum_distributable = (
        minimum_distributable * finance["construction_ownership"]
    )
    forward_distributable = max(0.0, forward_fcff - interest)
    catl_forward_distributable = (
        forward_distributable * finance["construction_ownership"]
    )
    catl_initial_equity = (
        capex.total_initial_capex_yi
        * (1 - finance["debt_ratio"])
    ...
    return SwapBusinessResult(
        ...
        labor_yi=labor,
        software_opex_yi=business["software_opex_yi_year"],
        ebitda_yi=ebitda,
