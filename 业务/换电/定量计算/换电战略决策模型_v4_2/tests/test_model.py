...
class StrategicModelTests(unittest.TestCase):
    ...
    def test_station_schedule_reconciles_and_finishes_by_2028(self) -> None:
        opening = self.config["construction"]["opening_2025_stations"]
        self.assertEqual(opening, {"heavy": 305, "choco": 1020})
        self.assertEqual(
            opening["heavy"]
        ...
        first = self.snapshot.capex.annual[0]
        targets_2026 = self.config["construction"]["station_2026_cumulative_targets"]
        self.assertEqual(first.new_heavy_stations, targets_2026["heavy"] - opening["heavy"])
        self.assertEqual(first.new_choco_stations, targets_2026["choco"] - opening["choco"])
    ...
    def test_baseline_mna_does_not_change_station_demand(self) -> None:
        ...
        self.assertEqual(self.snapshot.scale.target_station_demand["heavy"], 3563)
        # 巧克力站按「营运车需求÷规划能力」反推；私家车需求先填单站冗余
        # （物理上限552 − 规划300 = 252次/日），中枢档28.9万次/日远小于冗余总量
        # 7877×252≈198.5万次/日，故不新建站，站数由8840降为7877。
        self.assertEqual(self.snapshot.scale.target_station_demand["choco"], 7877)
    ...
    def test_station_capacity_changes_capex_not_demand(self) -> None:
        ...
        rent_rate = self.config["swap_business"]["battery_rent_rmb_kwh_year"]
        self.assertAlmostEqual(
            self.snapshot.swap_business.rent_vehicle_gwh * rent_rate / 100,
            rerun.swap_business.rent_vehicle_gwh * rent_rate / 100,
            places=8,
        )
        self.assertNotEqual(
            self.snapshot.swap_business.rent_station_external_gwh,
            rerun.swap_business.rent_station_external_gwh,
        )
    ...
    def test_short_life_replacement_is_split_20_80(self) -> None:
        """寿命改为按「2000次循环临界点÷年循环次数」推算后，首次更换提前到2028年。

        重卡长途（日均650km、f≈2.69次/日、背电513kWh）年循环≈942次，
        寿命=2000÷942≈2.12年 → 2026年装的批次在2028年触发首次更换（约10.1GWh）。
        这与《超换一体.md》§2.1「营运车高频→2.7-3.6年到临界点」一致（重卡长途更极端）。
        2026/2027因最短寿命仍>2年而无更换。
        """
        ...
        self.assertGreater(rows[2028].replacement_gwh, 0.0)
    ...
    def test_2026_cfo_uses_only_second_half(self) -> None:
        ...
        self.assertAlmostEqual(self.snapshot.funding[0].period_cfo_yi, expected)

    def test_mna_price_does_not_change_organic_power_value(self) -> None:
        changed = copy.deepcopy(self.config)
        base_name = changed["mna"]["base_scenario_name"]
        base_scenario = next(
            row for row in changed["mna"]["scenarios"]
            if row["name"] == base_name
        )
        base_scenario["purchase_price_yi"] += 10.0
        rerun = build_model(changed)
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            self.snapshot.funding[0].closing_liquid_resources_before_uncommitted_strategy_yi
            - rerun.funding[0].closing_liquid_resources_before_uncommitted_strategy_yi,
            10.0,
            places=8,
        )

    def test_terminal_residual_uses_common_price_but_batch_specific_age(self) -> None:
        short = terminal_residual_details(self.config, 2026, 3.8)
        long = terminal_residual_details(self.config, 2026, 10.0)
        self.assertAlmostEqual(short.horizon_price_ratio, long.horizon_price_ratio)
        self.assertNotEqual(short.terminal_batch_age_years, long.terminal_batch_age_years)
        self.assertNotEqual(short.terminal_residual_ratio, long.terminal_residual_ratio)

    def test_sensitivity_is_generated_from_core_model(self) -> None:
        groups = {row["parameter_group"] for row in self.snapshot.sensitivity}
        self.assertTrue({
            "运营EV/EBITDA", "制造PE", "服务费+租金",
            "资本回报要求CRF", "巧克力单站规划能力", "电池价格",
        }.issubset(groups))

    def test_report_template_does_not_store_output_results(self) -> None:
        template = (ROOT / "templates" / "strategic_report.md.tpl").read_text(
            encoding="utf-8"
        )
        for result in ("434.7", "6,760.8", "16,578.6", "211.8"):
            self.assertNotIn(result, template)
