...
| **定性内容外置 MD（沙盘）** | 沙盘①顶部结论、②定性逻辑三支柱的论证文案住 `narrative/沙盘结论区.md`（`# 顶部结论` 一段为模板、`# 定性逻辑` 下 `##` 分段）；`sandbox.py` 读 MD、`{key}` 占位符由模型算出的 `vals` 填；`templates/` 只含格式，**任何定性描述不进 `.py`**。改文案＝改 MD＋重生成 HTML（与一页纸同源） |
