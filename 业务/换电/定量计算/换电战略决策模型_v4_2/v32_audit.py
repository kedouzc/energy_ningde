...
def parameter_audit_rows(config: dict, snapshot: ModelSnapshot) -> list[tuple[str, ...]]:
    ...
    add("估值", "资管平台参数", "v3正文未形成独立可调三参数", f"费率{_pct(finance['manager_fee_rate'])} / 净利率{_pct(finance['manager_net_margin'],0)} / PE{finance['manager_pe']:g}×", "v4.2.1移除估值", "Part6改为两层估值后不再计算资管平台价值；参数仅存档，待真实滚动发行AUM出现才可能启用", "不再影响任何估值结果")
    ...
    add("需求", "私家电量/里程/电耗/寿命", "56kWh / 60km / 0.15 / 10年", f"{private['battery_kwh']:g}kWh / {private['daily_km']:g}km / {private['energy_consumption_kwh_km']:g} / {lives['private']:g}年", "按主文", "旧配置66.7km/90%与主文60km/80%冲突；v4.1跟主文；寿命改为推算后低频使循环寿命>10年、由日历封顶兜住10年", "频次、站数、CAPEX")
    ...
    for group, old_label in (("heavy", "重卡"), ("choco", "巧克力")):
        ...
        new_station = f"{station['station_body_capex_wan']:g}万/{station['inventory_blocks']:g}块×{station['block_kwh']:g}kWh/{snapshot.scale.station_battery_life_years[group]:g}年"
        add("站网", f"{old_label}站体/库存/寿命", old_station, new_station, "改为推算", "站体不含电池；站内电池寿命按本组所服务车型GWh加权推算", "站体与站内电池CAPEX")
    ...
    add("制造", "有/无换电净利率", "15% / 12.5%", f"{_pct(business['with_swap_manufacturing_net_margin'],1)} / {_pct(business['no_swap_manufacturing_net_margin'],1)}", "v4.2修正", "无换电12.5%→12.0%，净利率差锁定3pct；电动车制造盘定价权保护", "制造净利润与价值")
    ...
    add("集团资金", "已识别直接并购现金", "未纳入", f"2026年三项合计{_num(mna_2026_total,2)}亿元（中恒29.03＋世纪互联64.00＋启源{_num(base_mna_price,2)}）", "v4.2合并三项", "世纪互联按审慎口径补入；启源经并购基准情景进入资金表", "年度资金余量")
