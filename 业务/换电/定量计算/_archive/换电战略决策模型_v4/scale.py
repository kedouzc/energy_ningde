...
def build_scale(config: dict, sourcing: SourcingAdjustment) -> ScaleResult:
    ...
    for row in rows:
        daily = row.catl_swap_vehicles_wan * 1e4 * row.swap_frequency_per_day
        daily_swaps[row.station_group] += daily
        annual_energy_yi[row.station_group] += (
            daily * row.battery_kwh * usable * days / 1e8
        )
