...
from scale import BATTERY_POOLS, POOL_STATION_GROUP
from schemas import (
    BaselineResult,
    CapexResult,
    HeavyEconomics,
...
def build_heavy_economics(
    ...
    为什么这样拼（口径三条，写在这里以防后人拆开各取一项）：
    1. **TCO 是持有期全成本**，不是能源成本——含购车（扣补贴、含购置税）、维保、
       载重损失。公式复刻自 JPM Table 3，并用原表反算验证过
       （电动 595,000 + 276,250×8 = 2,805,000，与 Table 3 八年行完全一致）。
    2. **只替换能源单价那一项**：JPM 电动列用 0.85 元/kWh 的工商业充电价，换电列改用
       模型自己算的用户实付 P_swap =（服务费 + 电池租金）÷ 换电量。其余（购车/税/补贴/
       维保/载重损失）照搬 JPM 电动列，保证三者是同一套自洽参数下的对照。
    3. **换电专属两项单独计**：BaaS 免去的电池购置按「单车带电量 × 模型电池价」算
       （不用 JPM 的 40–50% 估计，因为电池价在模型里是曲线参数、要随调参变）；
       年增收 4 万（换电 vs 充电的时间价值，JPM §8.3）从年运营成本中扣。
...
def build_swap_business(config: dict, scale: ScaleResult, capex: CapexResult) -> SwapBusinessResult:
    ...
    return SwapBusinessResult(
        ...
        **_dcf_cross_check(config, capex, pool_ops, ebitda, forward_fcff, ownership, catl_value),
        ...
        # 【2026-09-10】重卡用户经济性（结论区「用户 TCO」一句取这里；缺 [tco_jpm] 为 None）
        heavy_economics=build_heavy_economics(config, scale, capex, pool_ops),
    )
