...
# 当前进度：[6. hy4 建模计算输出统一管理 - 20260911](#6-hy4-建模计算输出统一管理---20260911)
...
| **取数与注册**（2026-09-12 立，见坑 25–28） | **取不到必须闹出动静**：`read_metrics(snap, cfg, strict=True)` 路径不通／没传 cfg 即中断点名，NaN 只报一次不冒充；`load_metrics` 加载期核 `source` 与 `at` 尾段；**一个 key 一个定义**（`facts.py` 重复即中断）；`check_mirrors()` 下沉到 `build_facts`——**每条管线**断言手写事实与字典同值，含"mirror 指向不存在的 key"与"差 100 倍的比值 vs 百分数"两态；**孤儿即删**（65 条没人引用的事实与随之死掉的 7 个取值助手已删，`facts.json` 233 → 180 条，零 nan）；口径相近的两个数（574.0／574.2／608.8 GWh）各自有名字、有出处 |
...
# 6. hy4 建模计算输出统一管理 - 20260911

## 产品概述

把「模型到底算出了哪些数」从 Python 代码里独立出来，做成一本和 `configs/base.toml` 对称的**输出字典** `configs/metrics.toml`；在此基础上实现**中文取数**——写论述时只写中文含义，程序自己知道去取哪一个结果，写错就报错并给出最像的候选。

## 要解决的问题（用户原话转述）

改一个论述、要引用一个参数时，不该需要知道程序里的参数名叫什么。现在的做法是「绕了好几道弯儿」：既要在 MD 里框定参数，又要为这个 MD 适配建模程序，还要回头维护 `lab.py`。

## 核心需求

1. **输出字典独立可查**：`configs/metrics.toml`。一眼看全模型算出了哪些数：中文名、单位、精度、口径说明、取值路径。
2. **字典里不许有公式**：所有需要计算的派生量，一律下沉回跟它关系最紧密的计算程序（车辆类→`scale.py`、资本类→`capex.py`、经营类→`business.py`、合并类→`consolidation.py`、资金类→`group_constraints.py`）。字典只做纯引用。
3. **合并成一套命名，不要别名**：一词一名。写错时程序报错并列出最像的 3 个候选，用户照着改 MD 即可，不存别名。
4. **所有取数处都能写中文**：正文占位符、一页纸的 `metric:`、`report_map.toml` 的 `owns/uses`。
5. **双向审计**：① 字典里每一条，程序里都得找得到；② 报告里每一个引用，字典里都得找得到。缺哪边都报错，不静默。
6. **两本字典配套**：`base.toml` 管输入（假设参数＋外部查来的事实），`metrics.toml` 管输出（模型算出来的结果）。

## 不在本轮范围

八章正文撰写、决策树下钻（⑥）工程实现、跟踪功能实现、`business.py` 两版互校断言、`交接.md` 文本修订、`src/` 物理分目录。


## 技术栈

沿用现有栈、不引入新依赖：Python 3.11+（`tomllib`/dataclass）+ `openpyxl` + 单文件离线 HTML / 原生 JS / Pyodide。不引入数据库、不引入前端框架与 CDN。

## 实现思路（一句话）

**把"登记哪些结果"从代码搬到配置，把"怎么算"从配置搬回代码**——字典变成纯声明，计算回到它该在的模块；然后在这本字典上做中文寻址与双向审计。

```mermaid
graph LR
    subgraph 输入字典
        A["configs/base.toml<br/>假设参数 + 外部查来的事实"]
    end
    subgraph 计算程序
        B["scale.py / capex.py / business.py<br/>consolidation.py / group_constraints.py<br/>（所有计算都在这里）"]
    end
    subgraph 输出字典
        C["configs/metrics.toml<br/>纯声明：中文名 / 单位 / 精度 / 口径 / 取值路径"]
    end
    subgraph 出报告
        D["MD 写中文 {{年换电交易电量}}"]
        E["一页纸 metric: 中文"]
        F["report_map.toml owns/uses 中文"]
    end
    A --> B
    B -->|"把算好的量挂到快照"| S["ModelSnapshot"]
    C -->|"只引用，不计算"| S
    S --> C
    D --> R["中文寻址 resolve()"]
    E --> R
...
[[metric]]
key      = "swap.ebitda"
label    = "EBITDA"              # 唯一中文名，MD 里就写这个
at       = "swap_business.ebitda_yi"
unit     = "亿元"
decimals = 1
group    = "④运营财务"
note     = ""
...
key      = "ops.veh_ops"
label    = "营运车合计"
at       = "scale.veh_ops_total"   # 求和在 scale.py 里做，这里只引用
unit     = "万辆"
decimals = 1
group    = "①运营规模"
...
def resolve(name: str) -> Metric:
    """按 key / 中文名 / 归一化后的模糊匹配 找指标。
    找不到 → 抛错并带最像的 3 个候选（不静默、不存别名）。"""
```

报错形态（面向用户）：

```
✗ 3_目标市场.src.md 第 12 行：「重卡换电量」在输出字典里找不到
  最像的是：
    1. 重卡TCO·换电(N1持有期)   tco.swap_wan
    2. 年换电交易电量           ops.annual_energy
    3. 重卡装机保有量·车端       ops.heavy_vehicle_gwh
  改成上面任一个即可；确实缺这个数，就在 configs/metrics.toml 登记。
```
...
- **基线数字必须一字不变**：三情景可归因换电增量 **913.7 / 9,703.7 / 33,597.9 亿**；EBITDA 覆盖倍数 **0.81 / 1.17 / 1.51**；决策树 166 节点三情景全部通过；一页纸 33 行四条校验通过；结论区 41 个占位符全部取到值。
- **派生下沉每挪一个就跑一次验证**，数字不动才算过（这是本方案最大的风险点，必须小步快跑）。
- **分两批下沉**：先车辆/规模类（改 `scale.py`），再资本/经营/合并/资金/配置类。每批独立验证。
- **字典与代码的一致性**：`metrics.toml` 是唯一真相，`lab.py` 不得再出现第二份登记；`METRIC_BY_KEY` 由字典加载后构建。
- **兼容过渡**：`resolve()` 同时认 key 与中文名，所以现有 MD 不会一次性全坏，可以逐份迁移。
- **不改既有行为**：任何位置不出现"弹性外推 / ±30% / ≈"；档位态取 `D.tierValues` 精确实跑值；`templates/sandbox.js` 取元素一律先判空、`window.onerror` 保留。
- **留痕**：本轮每个改动配 DECISIONS 条目。
...
### SubAgent
- **code-explorer**
  - 用途：动手前穷尽列出 `src/lab.py` 里全部派生函数（`_stock_vehicles` / `_flow` / `_repl_gwh` / `_market_share` / `_heavy_pool_sum` / `_veh_ops` / `_total_gwh_2030` / `_operating_market_total` / `_reit_multiple` / `_energy_unit_price` / `_share_of_market` / `_repl_share_pct` / `_heavy_pen_pct` / `_peak_cash_to_cfo` / `_closing_liquidity` / `val.increment_gross` 等约 30 个），逐个给出：现在取哪些快照字段、应下沉到哪个模块（`scale`/`capex`/`business`/`consolidation`/`group_constraints`/`config_loader`）、建议在 `schemas.py` 加的字段名与类型、以及被哪些下游（verdict / onepager / report_map / 一页纸 / sandbox）引用。
  - 预期产出：「派生下沉对照表」（函数 → 目标模块 → 新字段名 → 下游引用点），作为分批下沉的唯一施工依据。
...
- [ ] 用 [subagent:code-explorer] 列出全部派生函数，产出「下沉对照表」（函数→目标模块→新字段名→下游引用点）
- [ ] 把车辆/规模类派下沉到 src/scale.py 与 schemas.py（营运车合计、各车种口径、出货口径、站数、日换电次数），每挪一个跑一次验证
- [ ] 把资本/经营/合并/资金/配置类派生下沉到 capex.py、business.py、consolidation.py、group_constraints.py（含更新装机、市场占比、重卡池、资金峰值、REIT 倍数、合并增量），每挪一个跑一次验证
- [ ] 建 configs/metrics.toml 输出字典（纯声明，无公式），改 src/lab.py 为加载器＋通用取值器，用 [skill:falsifiability-simplicity] 定字段规范
- [ ] 实现中文寻址 resolve()（key→中文名→归一化模糊），接入 src/inject.py 占位符解析；失败即报错并列出最像的 3 个候选，不静默不存别名
- [ ] 用 [skill:mwsm-single-model] 统一命名空间：一页纸 metric: 与 report_map.toml 的 owns/uses 支持中文；建双向审计（字典每条在快照中存在、报告每个引用在字典中存在）；facts 带 mirror 项回归、外部事实归 base.toml
- [ ] 端到端验证：run.py/build.py/sandbox.py 全绿且三情景数字不变（913.7 / 9,703.7 / 33,597.9 亿）；更新 src/README.md、框架提案.md、chapters/README.md；补 DECISIONS 条目
