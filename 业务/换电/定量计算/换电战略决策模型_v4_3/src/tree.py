...
from lab import load_param_docs, METRICS  # noqa: E402
...
# ─────────────────────────────────────────── 结果注册表：一个数只有一个出处
# lab.METRICS（configs/metrics.toml）是模型结果层的**唯一注册表**。树里每个
# 「直接取模型实跑值」的叶节点，名称和单位都该从那里按快照路径取，不应该在
# 本文件再手写一遍（那正是 2026-09-12 之前 tree.py / report.py 各自登记、口径
# 漂移的病根）。missing 即中断，逼着去 metrics.toml 补一条，而非在本文件补文字。
METRIC_BY_AT = {m.at: m for m in METRICS if m.at}


def M(key, path, *, formula="", label=None, unit=None, tol=1e-9, tol_note="",
      source=None, param=None, combine=None, children=None) -> Node:
    """叶 metric 节点：口径（名称/单位）从结果注册表按快照路径取，本文件不再登记。

    仅用于「值 = 模型实跑值（单一快照路径）」的叶节点；需要求和/比值/跨池计算的
    派生量仍用 `N(...)` 自带公式（那些口径在 metrics.toml 的 `note` 里说清）。
    """
    metric = METRIC_BY_AT.get(path)
    if metric is None:
        raise SystemExit(
            f"✗ tree 叶节点 {key!r} 的快照路径 {path!r} 不在 configs/metrics.toml 注册表里——"
            f"请先在 metrics.toml 补一条 [[metric]]（at={path!r}），"
            f"不要在本文件手写 label/unit（一个数只有一个出处）。")
    return N(key,
             label if label is not None else metric.label,
             unit if unit is not None else (metric.unit or ""),
             lambda c, _p=path: float(c.m(_p)),
             formula=formula, combine=combine, children=children or [],
             param=param, source=source, tol=tol, tol_note=tol_note)
