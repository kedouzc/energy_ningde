...
def _station_schedule(
    target: int,
    years: list[int],
    first_year_target: int,
    completion_year: int,
) -> list[int]:
    """2026先达到规划里程碑，剩余需求站数在2028年前完成。"""
    first = min(target, first_year_target)
    build_years = [year for year in years if years[0] < year <= completion_year]
    remaining = target - first
    schedule = {year: 0 for year in years}
    schedule[years[0]] = first
    built = first
    for index, year in enumerate(build_years):
        years_left = len(build_years) - index
        addition = remaining if years_left == 1 else round(remaining / years_left)
        schedule[year] = addition
        remaining -= addition
        built += addition
    if built != target:
        schedule[completion_year] += target - built
    return [schedule[year] for year in years]
...
def build_capex(
    ...
    station_body_total = sum(item["station_body"] for item in initial_components.values())
    ...
    total_initial = sum(sum(item.values()) for item in initial_components.values())
