...
from scale import BATTERY_POOLS, POOL_STATION_GROUP, _operating_stocks
...
def _battery_life_sections(config: dict, snapshot: ModelSnapshot) -> tuple[str, str]:
    ...
    for pool_key, pool_label in BATTERY_POOLS.items():
        life = scale.battery_pool_life_years[pool_key]
        old = LEGACY_V32_STATION_LIFE_YEARS[POOL_STATION_GROUP[pool_key]]
        rows.append((
            f"{pool_label}（站内+装车共池）", "—", "—",
            f"{life:.2f}", f"{old:.2f}", f"{life-old:+.2f}",
            f"{mult(old):.3f}→{mult(life):.3f}",
        ))
    ...
    p_life = life_of(p_f)
    c_life = scale.battery_pool_life_years["choco25_passenger"]
    note = (
        ...
        f"巧克力25#乘用车池因混入大量低频私家车，池内使用强度被摊薄，寿命延长到{c_life:.2f}年，"
        f"反而比v3.2的3.8年更长。"
...
def _life_mode_comparison(
    ...
    metrics: list[tuple[str, Callable[[ModelSnapshot], float], int, str]] = [
        ("骐骥75#倒短池寿命(年)",
         lambda s: s.scale.battery_pool_life_years["qiji75_short"], 2, "短途场景"),
        ("骐骥75#干线池寿命(年)",
         lambda s: s.scale.battery_pool_life_years["qiji75_trunk"], 2, "中途+长途场景"),
        ("巧克力25#乘用车池寿命(年)",
         lambda s: s.scale.battery_pool_life_years["choco25_passenger"], 2, "营运+私家共池"),
        ("巧克力35#城配池寿命(年)",
         lambda s: s.scale.battery_pool_life_years["choco35_city"], 2, "城配高频子集"),
    ...
    note = (
        f"寿命口径变更的净效应是**方向相反的两种力量相抵**：骐骥75#由"
        f"{LEGACY_V32_VEHICLE_LIFE_YEARS['heavy']}年一档分化为倒短池"
        f"{snapshot.scale.battery_pool_life_years['qiji75_short']:.2f}年/干线池"
        f"{snapshot.scale.battery_pool_life_years['qiji75_trunk']:.2f}年"
        f"（干线重置更密、资本更重；倒短池因使用强度低反而拉长），"
        f"而巧克力25#乘用车池由{LEGACY_V32_STATION_LIFE_YEARS['choco']}年延长到"
        f"{snapshot.scale.battery_pool_life_years['choco25_passenger']:.2f}年（重置更少、资本更轻）。"
...
def _private_scenario_tables(
    ...
    def _operating_vehicles(snap):
        # 营运车 = 全部车型扣除私家车；营运车是换电价值的基本盘，故排在表首
        d = snap.scale.operating_stock_by_vehicle_wan
        return sum(v for k, v in d.items() if k != "private")

    def _operating_gwh(snap):
        return sum(r.catl_swap_gwh for r in snap.scale.rows if r.vehicle_key != "private")

    rows: list[tuple[object, ...]] = [
        ("营运车CATL换电车辆（万辆）", *cell(_operating_vehicles),
         "重卡+城配+出租/网约/Robotaxi，换电价值基本盘"),
        ("营运车装机（GWh）", *cell(_operating_gwh),
         "车辆侧换电电池装机，含全部营运车型"),
...
def render_report(
    ...
    pool_short_labels = {
        "qiji75_short": "骐骥75#倒短",
        "qiji75_trunk": "骐骥75#干线",
        "choco25_passenger": "巧克力25#",
        "choco35_city": "巧克力35#",
    }
    stocks_base = _operating_stocks(config)
    swap_days = config["swap_business"]["operating_days"]
    swap_usable = config["swap_business"]["usable_energy_factor"]

    def _total_stock_text(key: str, vehicle: dict) -> str:
        """车型总车辆数：存量口径用保有量，净增口径（Robotaxi/私家车）用5年累计净增。"""
        if "stock_source" in vehicle:
            return _n(stocks_base[vehicle["stock_source"]], 1) + "（保有）"
        if "annual_net_additions_wan" in vehicle:
            return _n(sum(vehicle["annual_net_additions_wan"]), 1) + "（5年净增）"
        return _n(vehicle.get("stock_wan", 0.0), 1) + "（保有）"

    vehicle_base_rows = []
    for key in vehicle_order:
        vehicle = config["vehicles"][key]
        cycle_text = (
            f"{vehicle['replacement_cycle_years']:g}" 
            if "replacement_cycle_years" in vehicle else "净增口径"
        )
        for scene in vehicle["scenes"]:
            scene_rows = [
                row for row in scale.rows
                if row.vehicle_key == key and row.scene == scene["name"]
            ]
            if not scene_rows:
                continue
            terminal_vehicles = sum(
                row.catl_swap_vehicles_wan for row in scene_rows
            )
            terminal_gwh = sum(row.catl_swap_gwh for row in scene_rows)
            sample = scene_rows[-1]
            scene_energy_yi = (
                terminal_vehicles * 1e4
                * sample.swap_frequency_per_day
                * sample.onboard_battery_kwh
                * swap_usable * swap_days / 1e8
            )
            vehicle_base_rows.append((
                f"{vehicle['label']}·{scene['name']}",
                pool_short_labels[sample.battery_pool],
                _total_stock_text(key, vehicle),
                cycle_text,
                _n(sample.onboard_battery_kwh, 0),
                _n(sample.swap_frequency_per_day, 2),
                _n(terminal_vehicles, 1),
                _n(terminal_gwh, 1),
                _n(scene_energy_yi, 1),
            ))
    ...
    # 【新增｜各年车辆底座】车型×年份的EV/换电/充电车数与换电新增装机（含车型汇总行）。
    vehicle_year_rows = []
    for key in vehicle_order:
        vehicle = config["vehicles"][key]
        veh_rows = [row for row in scale.rows if row.vehicle_key == key]
        for year in scale.years:
            year_rows = [row for row in veh_rows if row.year == year]
            vehicle_year_rows.append((
                year,
                vehicle["label"],
                _n(sum(row.ev_vehicles_wan for row in year_rows), 1),
                _n(sum(row.catl_swap_vehicles_wan for row in year_rows), 1),
                _n(sum(row.catl_charge_vehicles_wan for row in year_rows), 1),
                _n(sum(row.catl_swap_gwh for row in year_rows), 1),
            ))
        vehicle_year_rows.append((
            "5年汇总", vehicle["label"],
            _n(sum(row.ev_vehicles_wan for row in veh_rows), 1),
            _n(sum(row.catl_swap_vehicles_wan for row in veh_rows), 1),
            _n(sum(row.catl_charge_vehicles_wan for row in veh_rows), 1),
            _n(sum(row.catl_swap_gwh for row in veh_rows), 1),
        ))
    vehicle_year_table = _table(
        [
            "年份", "车型", "EV车(万辆/年)", "CATL换电车(万辆/年)",
            "CATL充电车(万辆/年)", "换电新增装机(GWh/年)",
        ],
        vehicle_year_rows,
    )
    # 【新增｜四池底座汇总】四池（=四站型）口径的车辆/装机/寿命/需求/电量/站数汇总，
    # 是"4类车各用各站、电池不混流"架构的数据底座。
    pool_base_rows = []
    for pool_key in BATTERY_POOLS:
        pool_rows = [row for row in scale.rows if row.battery_pool == pool_key]
        pool_vehicles = sum(row.catl_swap_vehicles_wan for row in pool_rows)
        pool_vehicle_gwh = sum(row.catl_swap_gwh for row in pool_rows)
        station_cfg = config["stations"][pool_key]
        pool_station_gwh = (
            scale.target_station_demand[pool_key]
            * station_cfg["inventory_blocks"]
            * station_cfg["block_kwh"] / 1e6
        )
        pool_base_rows.append((
            pool_short_labels[pool_key] + f"（{pool_key}）",
            _n(pool_vehicles, 1),
            _n(pool_vehicle_gwh, 1),
            _n(pool_station_gwh, 1),
            _n(scale.battery_pool_life_years[pool_key], 2),
            _n(scale.mature_daily_swaps[pool_key] / 1e4, 1),
            _n(scale.mature_annual_energy_yi_kwh[pool_key], 1),
            _n(scale.target_station_demand[pool_key]) + "座",
        ))
    pool_base_rows.append((
        "总计",
        _n(sum(r.catl_swap_vehicles_wan for r in scale.rows), 1),
        _n(sum(r.catl_swap_gwh for r in scale.rows), 1),
        _n(sum(
            scale.target_station_demand[pk]
            * config["stations"][pk]["inventory_blocks"]
            * config["stations"][pk]["block_kwh"] / 1e6
            for pk in BATTERY_POOLS
        ), 1),
        "—",
        _n(sum(scale.mature_daily_swaps.values()) / 1e4, 1),
        _n(sum(scale.mature_annual_energy_yi_kwh.values()), 1),
        _n(sum(scale.target_station_demand.values())) + "座",
    ))
    ...
    vehicle_base_note = (
        "总车辆数为车型底座口径：存量车型（重卡/城配/出租/网约）用2030保有量，"
        "净增车型（Robotaxi/私家车）用2026—2030累计净增；装车电量按场景实际配置"
        "（重卡短途342/中长途513、乘用56、城配81kWh），与换电频次分母同源。"
    )
    # 【重构｜4站型】站点层大表：先列示四个站型（与四池1:1对应，各用各站、电池不混流），
    # 再合计为两大类（重卡=短途+中长途、巧克力=乘用+城配），与 v3.2 口径衔接。
    pool_station_labels = {
        "qiji75_short": "骐骥短途站",
        "qiji75_trunk": "骐骥干线站",
        "choco25_passenger": "巧克力乘用站",
        "choco35_city": "巧克力城配站",
    }
    opening_by_pool = capex.opening_station_stock_by_pool
    target2026_by_pool = capex.station_2026_targets_by_pool
    station_demand_rows = []
    for pool_key in BATTERY_POOLS:
        station_demand_rows.append((
            pool_station_labels[pool_key],
            _n(scale.station_capacity_diagnostics[pool_key]["mechanical_limit"], 0),
            _n(scale.station_capacity_diagnostics[pool_key]["energy_limit"], 0),
            _n(scale.station_capacity_diagnostics[pool_key]["planning_capacity"], 0),
            _n(scale.mature_daily_swaps[pool_key], 0) + "次",
            _n(scale.target_station_demand[pool_key]) + "座",
            _n(opening_by_pool.get(pool_key, 0)) + "座",
            _n(target2026_by_pool.get(pool_key, 0)) + "座",
            str(config["construction"]["station_network_completion_year"]),
        ))
    category_demand = scale.station_demand_by_category
    category_swaps = scale.daily_swaps_by_category
    station_demand_rows.extend([
        (
            "重卡合计（短途+干线）",
            "—", "—", "—",
            _n(category_swaps["heavy"], 0) + "次",
            _n(category_demand["heavy"]) + "座",
        ...
        (
            "巧克力合计（乘用+城配）",
            "—", "—", "—",
            _n(category_swaps["choco"], 0) + "次",
            _n(category_demand["choco"]) + "座",
        ...
        (
            "总计",
            "—", "—", "—",
            _n(sum(category_swaps.values()), 0) + "次",
            _n(sum(category_demand.values())) + "座",
            _n(sum(config["construction"]["opening_2025_stations"].values())) + "座",
            _n(sum(config["construction"]["station_2026_cumulative_targets"].values())) + "座",
            str(config["construction"]["station_network_completion_year"]),
        ),
    ])
    ...
    # 【重构｜4站型】单站经济账按四个站型分别计算（池键=站型键），
    # 每个站型用自己的站体参数与所属池寿命，不再做组内加权。
    for pool_key in BATTERY_POOLS:
        ...
        # 【重构｜4站型】站内电池寿命=所属池寿命（池内EFC分母已含站内库存）。
        capital_multiplier = lifecycle_factors(
            config,
            config["construction"]["years"][0],
            scale.battery_pool_life_years[pool_key],
        ).capital_multiplier
        lifecycle_capital = body_capex + battery_capex * capital_multiplier
        arbitrage = (
            station_battery_gwh
        ...
        # 单站满负荷经济账：服务费按外生规划能力满打满算，人工按站固定；两项合计为满负荷EBITDA。
        ...
        # v4.3：人工按站固定计价（骐骥重卡系 2 人双班 26.88 万/年；巧克力系无人值守 0）
        full_load_labor_wan = (
            swap_cfg["heavy_station_labor_wan_year"]
            if pool_key.startswith("qiji75")
            else swap_cfg["passenger_station_labor_wan_year"]
        )
        ...
        station_unit_rows.append((
            ...
            _n(site_rent, 2),
            _n(ebitda_delta, 2),
            _n(station_unit_metrics[pool_key]["value_delta"], 2),
        ))

    def _wan(value: float) -> str:
        return _n(value, 1) + "万元"

    # 【重构｜4站型】单站经济账按四个站型各列一列（参数代入只保留跨站型共享参数，
    # 站型专属参数直接体现在各列数值中）；旧版"骐骥/巧克力"两列在4站型下会
    # 错把干线站当巧克力站，已废弃。
    econ_by_pool = {row["pool"]: row for row in station_economics_rows}

    def _econ_row(field: str) -> tuple:
        """取四个站型某字段的取值列（顺序=BATTERY_POOLS）。"""
        return tuple(
            _n(econ_by_pool[pk][field], 1) for pk in BATTERY_POOLS
        )

    station_economics_table = _table(
        [
            "计算项目", "公式", "参数代入",
            "骐骥短途站", "骐骥干线站", "巧克力乘用站", "巧克力城配站",
        ],
        [
            (
                "① 站体初装投资", "外生输入（不含站内电池）", "—",
            ...
            (
                "② 电池库存初装投资",
                "库存块数×单块电量×电池价格",
                f"电池价{_n(price_2026, 0)}元/kWh；库存：骐骥24×171kWh、乘用14×56kWh、城配14×81kWh",
                *_econ_row("battery_wan"),
            ),
            (
                "③ 单站初装CAPEX", "①＋②", "—",
                *tuple(
                    _wan(
                        econ_by_pool[pk]["body_wan"] + econ_by_pool[pk]["battery_wan"]
                    ) for pk in BATTERY_POOLS
                ),
            ),
            (
                "④ 单站全周期资本",
                "站体＋电池×全周期资本倍数（含历次更新）",
                f"池寿命倒短{scale.battery_pool_life_years['qiji75_short']:g}/干线{scale.battery_pool_life_years['qiji75_trunk']:g}"
                f"/乘用{scale.battery_pool_life_years['choco25_passenger']:g}/城配{scale.battery_pool_life_years['choco35_city']:g}年"
                f"（四池口径，站内电池=所属池寿命）",
                *_econ_row("lifecycle_capital"),
            ),
            (
                "⑤ 满负荷服务费收入/年",
                "规划日次×运营天数×单次净电量×服务费",
                f"规划能力：骐骥192次/日、巧克力300次/日；{swap_cfg['operating_days']:g}天"
                f"×{_p(swap_cfg['usable_energy_factor'], 0)}×{swap_cfg['service_fee_rmb_kwh']:g}元",
                *_econ_row("full_load_service_wan"),
            ),
            (
                "⑥ 满负荷人工运维/年",
                "按站固定（骐骥2人双班/巧克力无人值守）",
                f"骐骥{swap_cfg['heavy_station_labor_wan_year']:g}万/站年；巧克力{swap_cfg['passenger_station_labor_wan_year']:g}万",
                *_econ_row("full_load_labor_wan"),
            ),
            (
                "⑦ 需求侧EBITDA/年", "⑤－⑥", "—",
                *_econ_row("demand_side_ebitda_wan"),
            ),
            (
                "⑧ 资产侧EBITDA/年",
                "外部权益租金＋套利－场租",
                f"外部权益{_p(1.0 - config['finance']['construction_ownership'], 0)}；"
                f"场租{config['swap_business']['site_rent_wan_year']:g}万元/站",
                *_econ_row("asset_side_ebitda_wan"),
            ),
            (
                "⑨ 满负荷单站EBITDA/年", "⑦＋⑧", "—",
                *_econ_row("full_load_ebitda_wan"),
            ),
            (
                "⑩ 年资本要求", "④×CRF", f"CRF {_p(crf, 1)}",
                *_econ_row("annual_capital_charge_wan"),
            ),
            (
                "⑪ 盈亏平衡利用率",
                "⑩÷⑨",
                "利用率低于此值则单站不覆盖资本要求",
                *tuple(
                    _p(econ_by_pool[pk]["breakeven_utilization"])
                    if econ_by_pool[pk]["breakeven_utilization"] else "不适用"
                    for pk in BATTERY_POOLS
                ),
            ),
            (
                ...
                "不含建设期爬坡",
                *tuple(
                    _n(econ_by_pool[pk]["full_load_payback_years"], 1) + "年"
                    if econ_by_pool[pk]["full_load_payback_years"] else "不适用"
                    for pk in BATTERY_POOLS
                ),
            ),
        ],
    ...
    station_economics_note = (
        "满负荷口径指按外生规划能力（骐骥192次/日、巧克力300次/日）打满，这是单站经济上界；"
        "盈亏平衡利用率才是单站生死线：骐骥短途/干线站利用率只要约"
        f"{_p(econ_by_pool['qiji75_short']['breakeven_utilization'], 0)}/"
        f"{_p(econ_by_pool['qiji75_trunk']['breakeven_utilization'], 0)}即可覆盖CRF资本要求，"
        "巧克力乘用/城配站需要约"
        f"{_p(econ_by_pool['choco25_passenger']['breakeven_utilization'], 0)}/"
        f"{_p(econ_by_pool['choco35_city']['breakeven_utilization'], 0)}——几乎要按设计能力满负荷运转。"
    ...
    # 【重构｜4站型】v3.2 只有两大类站数，按当前池需求份额拆分到四站型
    # （最大余数法保证拆分后合计=大类原值），先列4站型、再合计为2大类。
    category_pools = {
        "heavy": ("qiji75_short", "qiji75_trunk"),
        "choco": ("choco25_passenger", "choco35_city"),
    }
    old_count_by_pool: dict[str, int] = {}
    for cat, pools in category_pools.items():
        cat_total = sum(scale.target_station_demand[p] for p in pools)
        raw = {
            p: old_station_targets[cat] * scale.target_station_demand[p] / cat_total
            for p in pools
        }
        floors = {p: int(v) for p, v in raw.items()}
        remainder = old_station_targets[cat] - sum(floors.values())
        by_frac = sorted(pools, key=lambda p: raw[p] - floors[p], reverse=True)
        for p in by_frac[:remainder]:
            floors[p] += 1
        old_count_by_pool.update(floors)
    station_delta_rows = []
    total_initial_delta = 0.0
    total_ebitda_delta = 0.0
    total_value_delta = 0.0
    cat_initial_delta = {"heavy": 0.0, "choco": 0.0}
    cat_ebitda_delta = {"heavy": 0.0, "choco": 0.0}
    cat_value_delta = {"heavy": 0.0, "choco": 0.0}
    cat_old_delta = {"heavy": 0, "choco": 0}
    for pool_key in BATTERY_POOLS:
        old_count = old_count_by_pool[pool_key]
        current_count = scale.target_station_demand[pool_key]
        delta = current_count - old_count
        metrics = station_unit_metrics[pool_key]
        ...
        cat = "heavy" if pool_key.startswith("qiji75") else "choco"
        cat_initial_delta[cat] += initial_delta
        cat_ebitda_delta[cat] += ebitda_delta
        cat_value_delta[cat] += value_delta
        cat_old_delta[cat] += delta
    ...
    for cat, label in (
        ...
        station_delta_rows.append((
            label,
            f"{old_station_targets[cat]:,}",
            f"{scale.station_demand_by_category[cat]:,}",
            f"{cat_old_delta[cat]:+,}",
            _n(cat_initial_delta[cat], 2),
            _n(cat_ebitda_delta[cat], 2),
            _n(cat_value_delta[cat], 2),
        ))
    station_delta_rows.append((
    ...
    # 【重构｜池归属+寿命两列】资本因子表按四池逐行列示：第1列池归属、第2列该池
    # 电池寿命（原"2.94年（骐骥干线池）"合写一列拆开，避免读者无法判断
    # "2.94年"是谁的寿命——干线池频次高→寿命最短）。
    capital_factor_pool_labels = {
        "qiji75_short": "骐骥倒短池",
        "qiji75_trunk": "骐骥干线池",
        "choco25_passenger": "巧克力乘用池",
        "choco35_city": "巧克力城配池",
    }
    capital_factor_rows = []
    for pool_key in BATTERY_POOLS:
        life = scale.battery_pool_life_years[pool_key]
        label = f"{life:g}年"
        # 2026年投产批次的资本倍数（池寿命键齐全时直取，缺失时按同口径现算兜底）。
        factor = capex.lifecycle_factor_by_life.get(label)
        if factor is None:
        ...
        capital_factor_rows.append((
            capital_factor_pool_labels[pool_key],
            label,
            _x(factor, 3),
    ...
    terminal_station_battery_gwh = sum(
        ...
        * config["stations"][pool]["inventory_blocks"]
        * config["stations"][pool]["block_kwh"] / 1e6
        for pool in BATTERY_POOLS
    )
    eac_multiple = capex.lifecycle_capital_base_yi / capex.total_initial_capex_yi
    # EAC 展示用的代表寿命档：站内电池按四池寿命登记——全部引用参数，
    # 对应 lifecycle_factor_by_life 的 key 格式为 f"{life:g}年"（capex.py 按 %g 格式登记）；
    # 私家日历封顶档无对应cohort时直接现算，作为长寿命参照。
    pool_lives = scale.battery_pool_life_years
    private_life = float(config["battery_life_model"]["calendar_cap_years"])
    private_factor = lifecycle_factors(
        config, config["construction"]["years"][0], private_life
    ).capital_multiplier
    pool_factor_texts = []
    for _pool_key, _short_label in (
        ("qiji75_short", "倒短"),
        ("qiji75_trunk", "干线"),
        ("choco25_passenger", "乘用"),
        ("choco35_city", "城配"),
    ):
        _life = pool_lives[_pool_key]
        _key = f"{_life:g}年"
        _factor = capex.lifecycle_factor_by_life.get(_key, 0.0)
        pool_factor_texts.append(
            f"{_short_label}{_life:.2f}年{_x(_factor, 2)}"
        )
    pool_factor_text = "、".join(pool_factor_texts)
    capex_summary = _table(
        ...
        [
            ...
            (
                ...
                f"代表寿命档倍数（引用参数非硬编码）：四池{pool_factor_text}、"
                f"私家日历封顶{private_life:g}年（参照档）{_x(private_factor, 2)}",
    ...
    # 【重构｜4站型列】经营链大表在"结果"前列示四个站型（池键=站型键），
    # "结果"=四站型合计（swap 总量字段即四池之和，business.py 分池构建保证）。
    # 各站型列数值不带单位，与"结果"列同单位（次数/电量/金额按行自明）。
    pool_ops = swap.pool_operations

    def _pool_vals(attr: str, digits: int = 1) -> list[str]:
        return [_n(getattr(pool_ops[pk], attr), digits) for pk in BATTERY_POOLS]

    def _pool_ratio_vals(numer_attr: str, denom_attr: str, digits: int = 2) -> list[str]:
        cells = []
        for pk in BATTERY_POOLS:
            numer = getattr(pool_ops[pk], numer_attr)
            denom = getattr(pool_ops[pk], denom_attr)
            cells.append(_n(numer / denom, digits) if denom else "—")
        return cells

    operating_pool_headers = [pool_station_labels[pk] for pk in BATTERY_POOLS]
    operating_table = _table(
        ["经营链节点", "核心参数/公式", "参数值", *operating_pool_headers, "结果"],
        [
            ...
            ("▶ 现金OPEX", "⑫＋⑬＋⑭＋⑮＋⑯", "—", *_pool_vals("opex_yi"), _n(swap.opex_yi, 1) + "亿元"),
            ("▶ EBITDA", "营业收入－现金OPEX", "—", *_pool_vals("ebitda_yi"), _n(swap.ebitda_yi, 1) + "亿元"),
            ("⑰ 折旧", "全周期资本倍数、期末残值与寿命共同推导", "非现金费用；按各池资本底座派生", *_pool_vals("depreciation_yi"), _n(swap.depreciation_yi, 1) + "亿元"),
            ("⑱ 债务利息", "全周期项目债务×债息", f"{_n(capex.project_debt_yi,1)}亿元×{_p(config['finance']['debt_interest_rate'])}", *_pool_vals("interest_yi"), _n(swap.project_interest_yi, 1) + "亿元"),
            ("▶ 税前利润", "EBITDA－折旧－利息，按站型独立计税", "亏损池可为负（税盾不跨池抵扣）", *_pool_vals("pre_tax_profit_yi"), _n(swap.pre_tax_profit_yi, 1) + "亿元"),
            ("▶ 项目净利润", "税前利润 − max(0,税前利润)×税率（亏损池不交税、亏损如实计入）", f"税率{_p(config['finance']['tax_rate'],0)}；税盾不跨池抵扣", *_pool_vals("project_net_profit_yi"), _n(swap.project_net_profit_yi, 1) + "亿元"),
            ("▶ CATL归母净利润", "项目净利润×CATL经济权益", _p(config['finance']['construction_ownership'],0), *_pool_vals("catl_net_profit_yi"), _n(swap.catl_attributable_net_profit_yi, 1) + "亿元"),
            ("▶ 项目EV", "EBITDA×EV/EBITDA", _x(config['finance']['swap_ev_ebitda'],0), *_pool_vals("enterprise_value_yi", 0), _n(swap.enterprise_value_yi, 1) + "亿元"),
    ...
    # 【重构｜4站型列】现金回报表同样在"结果"前列示四个站型；
    # 金额行"结果"=四站型合计，比率行（收益率/回收期）"结果"为全项目整体值。
    cash_return_table = _table(
        ["现金回报节点", "公式", *operating_pool_headers, "结果", "投资含义"],
        [
            ("资本要求FCFF", "全周期资本底座×CRF", *_pool_vals("required_fcff_yi"), _n(swap.required_fcff_yi,1)+"亿元", "含更新资本后的最低年资本回报要求"),
            ("最低项目可分派现金", "资本要求FCFF－项目债务利息", *_pool_vals("minimum_distributable_yi"), _n(swap.minimum_distributable_cash_yi,1)+"亿元", "与v3.2回收期同口径；项目债续作、未另扣本金摊还"),
            ("CATL最低年可分派现金", "最低项目可分派现金×40%经济权益", *_pool_vals("catl_minimum_distributable_yi"), _n(swap.catl_minimum_distributable_cash_yi,1)+"亿元", "保守回收期采用这一口径"),
    ...
    external_share = 1.0 - config["finance"]["construction_ownership"]
    ...
    margin_gap_pct = f"{(ledger.with_swap_manufacturing.net_margin - ledger.no_swap_manufacturing.net_margin) * 100:.1f}pct"
    bridge_table = _table(
        ...
        [
            ...
            (
                ...
                "是",
                "量差剩余部分＝充电段装机－无换电总装机；即换电虹吸走的充电电池，是换电对充电的真实量影响，随制造线净利Δ一起×PE计入主值（运营计网络EBITDA、制造计电池销售损益，基础不同不算双算）",
            ),
            (
                "电动车制造盘利润率保护",
                ...
                "是",
                f"有换电制造收入×净利率差{margin_gap_pct}；本次仅含已建模电动车",
            ),
            (
                "制造线情景差额合计",
    ...
    performance_anchor_table = _table(
        ...
        [
            ...
            (
                "——其中：制造利润率保护价值",
                f"有换电制造收入×净利率差{margin_gap_pct}×PE",
                _n(ledger.manufacturing_swap_margin_effect_value_yi, 1) + "亿元",
            ),
            (
                "——其中：充电段虹吸净效应价值",
                "(充电段装机-无换电总装机)×纯制造净利率×PE",
                _n(ledger.manufacturing_charge_share_effect_value_yi, 1) + "亿元",
            ),
            (
                "稳态初装现金收益率",
    ...
    commitment_method_note = (
        ...
        "世纪互联买方虽为宁德时代非控制、非并表关联方，现仍按审慎口径与中恒电气、启源芯动力"
        "合并计入2026年三项已识别并购现金。"
    )
    nio_method_note = (
    ...
    station_impact_explanation = (
        ...
        f"骐骥站（短途/干线同参数）{config['stations']['qiji75_short']['inventory_blocks']:g}×"
        f"{config['stations']['qiji75_short']['block_kwh']:g}kWh带来的外部权益电池租金和套利可覆盖场租；"
        f"巧克力乘用/城配站{config['stations']['choco25_passenger']['inventory_blocks']:g}×"
        f"{config['stations']['choco25_passenger']['block_kwh']:g}/{config['stations']['choco35_city']['block_kwh']:g}kWh"
        "的两项收入低于固定场租。"
    ...
    # ---------- v4.3 必答问题层：四个决策问题各一张机器填数的答案表 ----------
    # 叙述层住在 MANIFEST.md（唯一事实源）；本表只给刻度，防止叙述与数字脱钩。
    total_swap_gwh = sum(row.catl_swap_gwh for row in scale.rows)
    peak_funding = max(snapshot.funding, key=lambda row: row.swap_equity_call_yi)
    qa_scale_table = _table(
        ["Q1 做透的规模", "数值", "口径"],
        [
            ("骐骥短途站", f"{capex.station_targets['qiji75_short']:,} 座", "需求反推"),
            ("骐骥干线站", f"{capex.station_targets['qiji75_trunk']:,} 座", "需求反推；重卡合计对照52号文3,000座交叉验证"),
            ("巧克力乘用站", f"{capex.station_targets['choco25_passenger']:,} 座", "需求反推"),
            ("巧克力城配站", f"{capex.station_targets['choco35_city']:,} 座", "需求反推"),
            ("CATL换电车辆", _n(sum(scale.operating_stock_by_vehicle_wan.values()), 1) + " 万辆", "四类车异构推算"),
            ("换电电池总装机", _n(total_swap_gwh, 1) + " GWh", "车端＋站内周转"),
            ("年换电交易电量（成熟期）", _n(swap.annual_energy_yi_kwh, 1) + " 亿kWh", "Σ车辆×频次×可用电量"),
        ],
    )
    qa_cost_table = _table(
        ["Q2 做透的代价", "数值", "口径"],
        [
            ("初装CAPEX合计", _n(capex.total_initial_capex_yi, 1) + " 亿元", "站体＋站内电池＋车端电池"),
            ("全周期资本投入现值（15年，7.5%折现）", _n(capex.lifecycle_capital_base_yi, 1) + " 亿元", "初装＋更新净额"),
            ("CATL建设期累计资本调用", _n(capex.catl_total_equity_call_yi, 1) + " 亿元", "40%经济权益×权益份额"),
            ("单年峰值资本调用", f"{_n(capex.catl_peak_equity_call_yi, 1)} 亿元（{capex.peak_year}年）", "年度排期推算"),
            ("CATL全周期权益承诺", _n(capex.catl_lifecycle_equity_commitment_yi, 1) + " 亿元", "含更新期权益投入"),
        ],
    )
    qa_value_table = _table(
        ["Q3 换回的价值", "数值", "口径"],
        [
            ("可归因换电增量价值", _n(ledger.total_swap_increment_value_yi, 1) + " 亿元", "有换电 vs 无换电总账差（下行两项之和）"),
            ("组成①：运营侧直接增量", _n(ledger.direct_swap_increment_value_yi, 1) + " 亿元", "服务费+租金+套利+辅助服务的运营价值×40%权益"),
            ("组成②：制造侧锁量锁价增量", _n(ledger.total_swap_increment_value_yi - ledger.direct_swap_increment_value_yi, 1) + " 亿元", "换电锁定出货×净利率差2.5pct×PE"),
            ("占当前A+H市值", _p(ledger.attributable_swap_value_to_current_group_market_cap, 1), f"基准市值{_n(config['financial_2026e']['group_market_cap_ah_yi'],0)}亿元"),
            ("2030E动力电池业务线价值", _n(ledger.power_value_2030_with_swap_yi, 1) + " 亿元", "制造＋运营两层"),
            ("其中：无换电纯制造对照", _n(ledger.power_value_2030_no_swap_yi, 1) + " 亿元", "情形B"),
            ("CATL年可分派现金（成熟期）", _n(swap.catl_forward_distributable_cash_yi, 1) + " 亿元", "正算FCFF－利息"),
            ("全投资回收期", _n(swap.catl_lifecycle_payback_years, 1) + " 年", "全周期权益承诺÷年可分派现金"),
        ],
    )
    qa_funding_table = _table(
        ["Q4 资金从容度", "数值", "口径"],
        [
            ("换电现金调用峰值", f"{_n(peak_funding.swap_equity_call_yi, 1)} 亿元（{peak_funding.year}年）", "当年权益资本调用"),
            ("峰值年换电现金/CFO", _p(peak_funding.swap_cash_to_cfo, 1), "对照体检线15%"),
            ("峰值年可投资金结余", _n(peak_funding.closing_liquid_resources_before_uncommitted_strategy_yi, 1) + " 亿元", "扣分红回购并购后"),
            ("集团最低流动性储备线", _n(config["group_funding"]["minimum_liquidity_yi"], 0) + " 亿元", "战略安全垫"),
            ("战略敞口合计（压力刻度）", _n(snapshot.strategic_exposure_yi, 1) + " 亿元", "换电＋固态＋其他承诺"),
            ("最终判断", snapshot.memos[-1].status, "决策备忘录链末端"),
        ],
    )
    ...
    values = {
        ...
        "choco_planning_capacity": _n(config["stations"]["choco25_passenger"]["planning_daily_capacity"], 0),
        ...
        "external_share_pct": _p(1.0 - config["finance"]["construction_ownership"], 0),
        ...
        "vehicle_base_table": vehicle_base_table,
        "vehicle_year_table": vehicle_year_table,
        "pool_base_table": pool_base_table,
        "vehicle_base_note": vehicle_base_note,
        ...
        "qa_scale_table": qa_scale_table,
        "qa_cost_table": qa_cost_table,
        "qa_value_table": qa_value_table,
        "qa_funding_table": qa_funding_table,
        "v32_audit_table": v32_audit_table,
        "v32_outcome_audit_table": v32_outcome_audit_table,
        "final_conclusion": final_conclusion,
    }
    template_path = ROOT.parent / "templates" / "strategic_report_v4_3.md.tpl"
